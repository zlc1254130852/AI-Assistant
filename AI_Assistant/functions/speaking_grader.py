...
from utils.transform import pcm2wav
from functions.AI_chat import openai_client
from api_key import DIFY_API_KEY_SPEAKING
from utils.stream_generation import generate_dify_speaking
import base64, json
from externals.assess2 import send_to_assess
...
def save_file():
    ...
    # send the transcript to DIFY for further assessment
...
def update3(req):
    ...
    if 'stop' in req:
        ...
        json.loads(result)['PronFluency'])+"\n\nWaiting for the response...\n\n"})
            ...
            # send to DIFY for further assessment
            headers = {'Authorization': DIFY_API_KEY_SPEAKING, 'Content-Type': 'application/json'}
            payload = {'inputs': {}, 'query': transcript, 'response_mode': 'streaming',
